...
function classifyError(status: number, error: unknown): ApiError {
  ...
  switch (status) {
    case 401: return { type: ApiErrorType.UNAUTHORIZED, message: '登录已过期', status: 401 };
    case 403: return { type: ApiErrorType.FORBIDDEN, message: '没有权限访问', status: 403 };
    case 429: return { type: ApiErrorType.RATE_LIMIT, message: '请求过于频繁，请稍后重试', status: 429 };
    default:
